...
void Initialize() {
  ...
  keymap_[SDL_SCANCODE_C] = MESSAGE::NEXT_PLAYER;
...
cmd_t Update() {
  ...
  logger::Log(logger::DEBUG, "mouse x, y: %d, %d", x, y);
  ...
  // messenger::BroadcastMessage(MESSAGE::CMD_MOUSELOOK, &MouseMoveMessage(rx,
  // ry));
  cmd_t cmds;
  memset(&cmds, 0, sizeof(cmds));
  if (keystate[SDL_SCANCODE_W])
    cmds.forward_move -= 125;
