...
Renderer::Renderer(void)
    : screen_width_(WIDTH), screen_height_(HEIGHT), current_shader_(nullptr) {}
...
glm::mat4 Renderer::GetCameraMatrixFromEntity(Entity &entity) {
  ...
  // glm::dot(-position_, look_), glm::dot(-position_, right_),
  // glm::dot(-position_, up_), 1.0f);

  return glm::mat4(right_ogl.x, up_ogl.x, look_ogl.x, 0.0f, right_ogl.y,
                   up_ogl.y, look_ogl.y, 0.0f, right_ogl.z, up_ogl.z,
                   look_ogl.z, 0.0f, glm::dot(-position_ogl, right_ogl),
                   glm::dot(-position_ogl, up_ogl),
                   glm::dot(-position_ogl, look_ogl), 1.0f);
}
...
void Renderer::Initialize() {
  ...
  projectionmatrix_ = glm::perspective(
      45.0f, (float)screen_width_ / (float)screen_height_, 1.0f, 10000.f);
  orthomatrix_ = glm::ortho(0.0f, (float)screen_width_, 0.0f,
                            (float)screen_height_, -1.0f, 1.0f);
}
...
void Renderer::Setup3DRendering() {
  ...
  // modelmatrix_ = glm::lookAt(glm::vec3( 50.0f, 50.0f, 50.0f ),
  // glm::vec3(-15.0, -15.0, -15.0), glm::vec3(0.0, 1.0, 0.0));
...
void Renderer::Setup2DRendering() { glDisable(GL_CULL_FACE); }
...
void Renderer::BindTexture(unsigned int texture_unit,
...
void Renderer::BlendFunc(unsigned int new_blend_func0,
                         unsigned int new_blend_func1) {
...
void Renderer::AlphaFunc(unsigned int new_alpha_func,
                         unsigned int new_alpha_val) {
...
void Renderer::RenderPolygon(bsp_face *face) {
  ...
  if (offset >= world.map_->num_vertexes_)
    return;
