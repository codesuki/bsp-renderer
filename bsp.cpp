...
void bsp::get_visible_faces(const glm::vec4& camera_position)
{
  ...
  for (int i = m_num_leafs-1; i >= 0; --i)
  {
    ...
    glm::vec3 min((float)m_leafs[i].mins[0], (float)m_leafs[i].mins[1], (float)m_leafs[i].mins[2]);
    glm::vec3 max((float)m_leafs[i].maxs[0], (float)m_leafs[i].maxs[1], (float)m_leafs[i].maxs[2]);
